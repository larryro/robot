...
class WechatYG
{
     ...
     * @return Result
     */
	private static function sendMediaMsg($fromUsername,$url,$type='voice', $messageId){
		// $fromUsername = 'ouHUtt-OpyLTpWIksy8u6d70WT40';
		$access_token = self::getAccessToken(self::$appId,self::$appSecret);
		
		//获取媒体ID 
		$media_id = self::uploadMedia($access_token, $url, $type, $messageId);
     ...
     * @return Result
     */
	public static function sendMsg($fromUsername, $url, $type='text', $messageId = 0)
	{
		// $fromUsername = 'ouHUtt-OpyLTpWIksy8u6d70WT40';
		if ($type == 'text') {
			...
			return self::sendMediaMsg($fromUsername, $url, $type, $messageId);
		}
	}
	...
	public static function sendTextMsg($fromUsername,$content){
		...
		$result= self::http_request($url,$postData);
		echo date('Y-m-d H:i:s',time()).' sendTextMsg '.$result."\n";
		$resultObj = json_decode($result);
		if ($resultObj->errcode == 0) {
			return true;
		}else{
     ...
     */
	public static function uploadMedia($access_token,$filepath,$type,$messageId){
		//过时的方法 $filedata = array("media" => "@".$filepath);
		// 设置缓存，100分钟更新一次access_token
		$mem = new Memcache;
		$mem->connect(self::$memHost, self::$memPort) or die ("Could not connect");
		$media_id = $mem->get($type.$messageId);
		if (empty($media_id)) {
			...
			if (array_key_exists('media_id', $jsoninfo)) {
				$mem->set($type.$messageId, $jsoninfo['media_id'], 0, 6000);
				$mem->close();
...
// Wechat::sendTextMsg('oDAkQs8C6NUiJNx3jErNUIdRfxmM','test');
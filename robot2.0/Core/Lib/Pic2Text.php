...
class Pic2Text
{
	...
	public static function getresult($url) {
		...
		$str=self::curl_post($header,$postData,$post_url);
		$pattern = '#Sentence:</h4><ul><li>(.*)</li></ul><br/><h4>#';
		$sentence=self::getPatternResult($pattern,$str);
		if($sentence=="error"){
			return "error";
		}
		else{
	...
	public static function translate($string) {
		...
		$str=self::http_request($url.$string);
		echo $url.$string;
	    // $str=mb_convert_encoding($str,'GB2312','UTF-8');
		$pattern = '#translation\":\[\"(.*)\"\]#';
		return self::getPatternResult($pattern,$str);
	}

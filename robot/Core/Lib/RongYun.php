...
class RongYun{
     ...
     * @param string $format
     */
    public static function ini($appKey,$appSecret,$format = 'json',$serverUrl){
        self::$appKey = $appKey;
     ...
     * @return json|xml
     */
    public static function  getToken($userId,$name,$portraitUri) {
        try{
            if(empty($userId))
            ...
            $ret = self::curl('/user/getToken',array('userId'=>$userId,'name'=>$name,'portraitUri'=>$portraitUri));
            if(empty($ret))
     ...
     * @return Result
     */
    public static function sendMsg($Id, $value, $type)
    {
        $sysId = 34;
        if ($type == 'text') {
        ...
        // 向用户发送相关消息
        self::messagePublish($sysId, array($value),$objectName,$value, $type );
    }
     ...
     * @return json|xml
     */
    public static function messagePublish($fromUserId,$toUserId = array(),$objectName,$content,$pushContent='',$pushData = '') {
        try{
            if(empty($fromUserId))
            ...
            $ret = self::curl('/message/publish',$paramsString);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function messageGroupPublish($fromUserId,$toGroupId = array(),$objectName,$content,$pushContent='',$pushData = '') {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            if(empty($toGroupId))
            ...
            $ret = self::curl('/message/group/publish',$paramsString);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function messageChatroomPublish($fromUserId,$toChatroomId = array(),$objectName,$content) {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            ...
            $ret = self::curl('/message/chatroom/publish',$paramsString);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function messageSystemPublish($fromUserId,$toUserId = array(),$objectName,$content,$pushContent='',$pushData = '') {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            if(empty($toUserId))
            ...
            $ret = self::curl('/message/system/publish',$paramsString);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function messageBroadcast($fromUserId,$objectName,$content) {
        try{
            if(empty($fromUserId))
                throw new Exception('发送人用户 Id 不能为空');
            if(empty($objectName))
            ...
            $ret = self::curl('/message/broadcast',array('fromUserId'=>$fromUserId,'objectName'=>$objectName,'content'=>$content));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function messageHistory($date) {
        try{
            if(empty($date))
                throw new Exception('时间 不能为空');
            $ret = self::curl('/message/history',array('date'=>$date));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    private static function messageHistoryDelete($date) {
        try{
            if(empty($date))
                throw new Exception('时间 不能为空');
            $ret = self::curl('/message/history/delete',array('date'=>$date));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function groupSync($userId,$data = array()) {
        try{
            if(empty($userId))
            ...
            $ret = self::curl('/group/sync',array('userId'=>$userId,"$groupInfo"=>$arrVal[0]));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function groupJoin($userId,$groupId,$groupName) {
        try{
            if(empty($userId))
                throw new Exception('被同步群信息的用户 Id 不能为空');
            if(empty($groupId))
            ...
            $ret = self::curl('/group/join',array('userId'=>$userId,"groupId"=>$groupId,'groupName'=>$groupName));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    private static function groupQuit($userId,$groupId) {
        try{
            if(empty($userId))
                throw new Exception('被同步群信息的用户 Id 不能为空');
            if(empty($groupId))
                throw new Exception('加入的群 Id 不能为空');
            $ret = self::curl('/group/quit',array('userId'=>$userId,"groupId"=>$groupId));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return mixed
     */
    private static function groupDismiss($userId,$groupId) {
        try{
            if(empty($userId))
                throw new Exception('操作解散群的用户 Id 不能为空');
            if(empty($groupId))
                throw new Exception('要解散的群 Id 不能为空');
            $ret = self::curl('/group/dismiss',array('userId'=>$userId,"groupId"=>$groupId));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function groupCreate($userId,$groupId,$groupName) {
        try{
            if(empty($userId))
            ...
            $ret = self::curl('/group/create',array('userId'=>$userId,"groupId"=>$groupId,'groupName'=>$groupName));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function chatroomCreate($data = array()) {
        try{
            if(empty($data))
                throw new Exception('要加入群的用户 Id 不能为空');
            $params = array();
            ...
            $ret = self::curl('/chatroom/create',$params);
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function chatroomDestroy($chatroomId) {
        try{
            if(empty($chatroomId))
                throw new Exception('要销毁的聊天室 Id 不能为空');
            $ret = self::curl('/chatroom/destroy',array('chatroomId'=>$chatroomId));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
     ...
     * @return json|xml
     */
    private static function chatroomQuery($chatroomId) {
        try{
            if(empty($chatroomId))
                throw new Exception('要查询的聊天室 Id 不能为空');
            $ret = self::curl('/chatroom/query',array('chatroomId'=>$chatroomId));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
    ...
    private static function curl($action,$params) {
        $action = self::$serverUrl.$action.'.'.self::$format;
        $httpHeader = self::createHttpHeader();
        $ch = curl_init();
